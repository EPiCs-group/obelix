...
class MACE:
    ...
    def find_bidentate(mol, metal_atom_atomic_number):
        ...
        try:
            ...
            metal_atom = [
                ...
                if atom.GetAtomicNum() == metal_atom_atomic_number
            ][0]
            ...
            for bond in metal_bonds:
                ...
                (
                    ...
                    if idx1 != metal_atom.GetIdx()
                    else bidentate_idxs.append(idx2)
                )
    ...
    def generate_complex_SP_xyz(self, auxiliary_ligands=[]):
        ...
        for i, X in enumerate(Xs):
            ...
            X.ToXYZ(
                self.CA + "_" + "{}_{}.xyz".format(self.name_of_xyz, i), confId="min"
            )
    ...
    def generate_complex_OH_xyz(self, auxiliary_ligands=[], substrate=[]):
        ...
        try:
            ...
            bidentate_cycle_idxs = (
                None  # all cycles that could possibly be the metal-bidentate cycle
            )
            bidentate_idxs = []  # the final bidentate cycle indices
            for i, X in enumerate(Xs):
                X.AddConformers(numConfs=20)
                X.ToXYZ(
                    self.CA + "_" + "{}{}.xyz".format(self.name_of_xyz, i), confId="min"
                )
        except Exception as e:
...
class Workflow:
    ...
    def __init__(
        ...
        if mace_input != []:
            (
                self.mace_ligands,
                self.auxiliary_ligands,
                self.geom,
                self.central_atom,
                self.names_of_xyz,
                self.substrate,
            ) = self.initialize_mace()
        if chemspax_input != []:
            self.substituent_list, self.path_to_database, self.path_to_substituents = (
                self.initialize_chemspax()
            )
        if crest_input != []:
            (
                self.method,
                self.charge_of_complex,
                self.multiplicity,
                self.solvent,
                self.conf_search,
            ) = self.initialize_crest()
        if descriptor_calculator_input != []:
            (
                self.descriptor_central_atom,
                self.descriptor_metal_adduct,
                self.descriptor_output_type,
                self.extract_xyz_from_log,
                self.descriptor_printout,
                self.descriptor_solvent,
            ) = self.initialize_descriptor_calculator()
    ...
    def initialize_descriptor_calculator(self):
        ...
        return (
            descriptor_central_atom,
            descriptor_metal_adduct,
            descriptor_output_type,
    ...
    def run_mace(self):
        ...
        for i in tqdm(range(len(self.mace_ligands))):
            try:
                if self.geom == "OH":
                    ...
                    complex = MACE(
                        self.mace_ligands[i], self.central_atom, identifier_OH
                    )
                    complex.generate_complex_OH_xyz(
                        self.auxiliary_ligands, substrate=self.substrate
                    )

                if self.geom == "SP":
                    identifier_SP = self.names_of_xyz[i] + "_SP"
                    complex = MACE(
                        self.mace_ligands[i], self.central_atom, identifier_SP
                    )
                    ...
                    for j in range(Xs):
                        if self.names_of_xyz[i] in self.mace_skeletons.keys():
                            self.mace_skeletons[self.names_of_xyz[i]].append(
                                "_".join(
                                    [self.central_atom, identifier_SP, str(j) + ".xyz"]
                                )
                            )
                        else:
                            self.mace_skeletons[self.names_of_xyz[i]] = [
                                "_".join(
                                    [self.central_atom, identifier_SP, str(j) + ".xyz"]
                                )
                            ]
        ...
        logging.info("object id of complex: %s", id(complex))
    ...
    def run_chemspax(self, names, functionalization_list):
        ...
        if self.substituent_list == []:
            dest_dir_skeletons = os.path.join(
                self.path_to_workflow, "ChemSpaX", "chemspax_output"
            )
            shutil.copytree(src_dir_skeletons, dest_dir_skeletons)

        else:
            dest_dir_skeletons = os.path.join(
                self.path_to_workflow, "ChemSpaX", "skeletons"
            )
            if not os.path.exists(dest_dir_skeletons):
                shutil.copytree(src_dir_skeletons, dest_dir_skeletons)
            skeleton_list = glob.glob(
                os.path.join(self.path_to_workflow, "ChemSpaX", "skeletons", "*.xyz")
            )
        ...
        for index, sub_list in enumerate(self.substituent_list):
            ...
            chemspax_skeletons = [
                os.path.join(path_to_skeletons, i)
                for i in self.mace_skeletons[names[index]]
            ]
            ...
            chemspax_main(
                ...
                self.path_to_substituents,
                os.path.join(path_to_skeletons),
                chemspax_working_directory,
                path_to_output,
            )
            ...
            for skeleton in chemspax_skeletons:
                os.rename(
                    ...
                    os.path.join(
                        ...
                        list(names)[index]
                        + "_functionalization_"
                        + str(functionalization_list[index])
                        + ".mol",
                    ),
                )
    ...
    def run_crest(
        self, path_to_complexes=[], path_to_output=[], conformer_search="off"
    ):
        """
        ...
        if path_to_complexes == []:
            path_to_complexes = os.path.join(
                self.path_to_workflow, "ChemSpaX", "chemspax_output"
            )
            os.chdir(self.path_to_workflow)
        ...
        for path_to_complex in glob.glob(os.path.join(path_to_complexes, "*.xyz")):
            ...
            if conformer_search == "on":
                subprocess.call(
                    'xtb "%s" --%s --chrg %s  --uhf %s --alpb %s --opt > xtb.out'
                    % (
                        path_to_complex,
                        self.method,
                        self.charge_of_complex,
                ...
                subprocess.call(
                    "crest xtbopt.xyz --%s --chrg %s  --uhf %s --alpb %s > crest.out"
                    % (
                        path_to_complex,
                        self.method,
                        self.charge_of_complex,
    ...
    def calculate_descriptors(self):
        ...
        if (
            self.descriptor_output_type == "xyz"
            or self.descriptor_output_type == "crest"
        ):
            descriptor_calculator = Descriptors(
                central_atom=self.descriptor_central_atom,
                path_to_workflow=self.path_to_workflow,
                output_type=self.descriptor_output_type,
            )

            descriptor_calculator.calculate_morfeus_descriptors(
                geom_type=geom_type,
                solvent=self.descriptor_solvent,
                printout=self.descriptor_printout,
                metal_adduct=self.descriptor_metal_adduct,
            )
            ...
            descriptor_calculator.calculate_dft_descriptors_from_log(
                geom_type=geom_type,
                solvent=self.descriptor_solvent,
...
if __name__ == "__main__":
    ...
    # MACE input
    mace_input = {
        "bidentate_ligands": ligand_excel_file,
    ...
    descriptor_input = {
        "central_atom": descriptor_central_atom,
    ...
    # print(skeleton_list)
    workflow = Workflow(
        path_to_workflow=os.getcwd() + "/Workflow",
        geom="SP",
        descriptor_calculator_input=descriptor_input,
    )
