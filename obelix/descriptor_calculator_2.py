...
class Descriptors:
    ...
    def calculate_morfeus_descriptors_ce(self, metal_adduct='pristine', filename = 'filename1', solvent=None, plot_steric_map=False,printout = False, ce_name = None):
        if self.ce is not None:
            ...
            big_df = None
            try:
                for conformer_idx, conformer in enumerate(self.ce.conformers):
                    elements, coordinates = self.ce.elements, conformer.coordinates
                    print("checkpoint2")
                    conformer.properties, metal_idx, bidentate_max_donor_idx, bidentate_min_donor_idx = self._calculate_steric_electronic_desc_morfeus(solvent=solvent, dictionary=conformer_properties, elements=elements, coordinates=coordinates, filename=filename, metal_adduct=metal_adduct, plot_steric_map=plot_steric_map)
                    print(conformer_properties)
...
if __name__ == "__main__":
    ...
    #dft_descriptors.calculate_dft_descriptors_from_log(solvent=None, extract_xyz_from_log=True, printout=False, metal_adduct='nbd', plot_steric_map=False)
    #dft_descriptors.descriptor_df.to_csv('DFT_descriptors.csv', index=False)

    #this is just to get a ce now:
    import pickle
    ...
    ce1 = ensemble_dict['ceL117']
    print(ce1.conformers)
    

    descriptors = Descriptors(central_atom='Rh', ce = ce1)
    descriptors.calculate_morfeus_descriptors_ce(metal_adduct= 'nbd', ce_name = "ceL117")
    descriptors.descriptor_df.to_csv("descriptor_df_crest_ceL117.csv", index = False)
